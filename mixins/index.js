...
export default {
  ...
  methods: {
    ...
    handleLocationChange(type = 'default') {
      const { longitude: MOClongitude, latitude: MOClatitude } = storage.get('MOCK_LOC');
      uni.getLocation({
        type: 'gcj02',
        success: (res) => {
          const { longitude, latitude } = res;
          const point = { longitude: MOClongitude || longitude, latitude: MOClatitude || latitude };
...
[all ? 'selectAll' : 'select'](selector)
    ...
    /**
     * 跳转路线查询插件
     * @param {*} pois = {} startPos和endPos是起点和终点的信息，各自包含经纬度及街道名称与街道uid
     * @param {*} type 起终点的传参类型
        0: 参数中不带起点和终点，以定位点为起点
        1: 参数中带起点
        2: 参数中带终点，以定位点为起点
        3: 参数中既带起点又带终点
     * @param {*} curTab 选中的tab项
        1: 驾车
        2: 公交
        3: 步行
        4: 骑行
     * @param {*} from 渠道号
     * @param {*} isFocus 是否需要将焦点置于终点输入框
     */
    handleRoutePlugin(pois = {}, type = 2, curTab = 1, from = 'scenic', isFocus = false) {
      const option = {
        type,
        curTab,
        from,
      ...
      this.handleCommonRoute(`/modules/route/pages/index/index?option=${JSON.stringify(option)}`);
    },
    /**
     * 返回上一页
     */
    ...
    handleCommonBeacon(type, value) {
      ...
      this.$beacon.onUserAction(type, value && !value.openid ? value : {
        openid,
        ...value,
      });
    },
