// 中文
export default {
  name: "浅子体验设计中心",
  brand: "PhotoArtLife",
  author: {
    star: "狮子座",
    position: "北京市朝阳区",
    founder:
      "一个创作者，PhotoArtLife、MUX Lab、浅子文化工作室创始人 | 独立设计师、软件工程师、摄影师、制片人、独立开发者、开源作者等。"
  },
  abstract:
    "我们基于用户和企业的一页日历 • AI日报 伴你每天好心情 ☀️\n精选全球视觉与摄影艺术创作，览尽天下英雄。",
  copyright: "@浅子文化工作室 ✘ PhotoArtLife ✘ MUX Lab",
  introduce:
    "我们的团队来自谷歌、腾讯、阿里、百度、小米、易道、区块链等互联网软件和服务行业，致力于为创作者提供小而美、聚合创作和分享的内容品牌营销服务。",
  services:
    "合作方向：平面设计、广告服务、影像拍摄及制作、组织艺术活动、互联网软件服务及设计研发、品牌营销及策划、数字货币交易及服务、社区运营及媒体合作等。",
  labels: [
    "创造力",
    "设计",
    "视觉",
    "摄影",
    "旅行",
    "创意",
    "动画",
    "创作者",
    "媒体",
    "探索",
    "软件",
    "代码",
    "产品",
    "制片人",
    "导演",
    "博主",
    "纪录者",
    "短视频",
    "视频",
    "影集",
    "记者",
    "工程师",
    "挑战",
    "实验室",
    "工作室",
    "时尚",
    "居家",
    "健康",
    "运动",
    "创业者",
    "市场",
    "区块链",
    "数字货币",
    "比特币",
    "艺术",
    "品牌",
    "业务",
    "人工智能",
    "策划",
    "营销",
    "广告",
    "色彩"
  ],
  tabs: ["主页", "社区", "创作"],
  team: [
    {
      name: "Mich",
      image:
        "https://tvax3.sinaimg.cn/crop.4.0.1116.1116.180/967d9727ly8fftw9b3jmmj20v90v0wgq.jpg",
      role: "创始人、创作者、摄影师等"
    },
    {
      name: "James",
      image:
        "http://wx.qlogo.cn/mmopen/lx6nJKHrPibX4TYXXrvNMVoeBf923oQx09OeCP8X7rSajibKtkuyJcticDhSQNhgvqKx46ngn9YHiciaPKLRJYQQY5kV7DRor4OYJ/0",
      role: "市场总监、增长黑客等"
    },
    {
      name: "Hepburn",
      image:
        "https://tvax4.sinaimg.cn/crop.0.0.1002.1002.180/007q1594ly8garlm6zz9aj30ru0rutc2.jpg",
      role: "工程师、户外探险爱好者等"
    },
    {
      name: "Asako Studio",
      image:
        "https://lz.sinaimg.cn/orj1080/967d9727ly3gc0whyiy96j20sg0sg1jj.jpg",
      role: "设计师、视觉艺术家等"
    },
    {
      name: "MUX lab",
      image:
        "https://cdn.dribbble.com/users/32512/screenshots/6495831/face_id2_effect_by_gleb.gif",
      role: "产品总监、投资人等"
    }
  ],
  footer: {
    buttons: ["分享", "联系"]
  }
};
