 ...
 * @version 0.1 | 2020-11-06 // fix findCurrentIndex bug(数据项为null)
 * @Date: 2020-11-06 09:56:19
 * @Last Modified by: mukuashi
 * @Last Modified time: 2020-11-17 22:14:42
 */
...
function dataCombineStack(series, len) {
  ...
  return series.reduce((a, b) => (a.data ? a.data : a).concat(b.data).concat(sum), []);
}
...
function findCurrentIndex(currentPoints, calPoints, opts, config) {
  ...
  let spacing = opts.chartData.eachSpacing / 2;
    ...
    const currentList = calPoints.flat().filter(e => e);
		if (opts.type == 'candle') {
			...
			for (let i = 0;i < calPoints[0].length;i++) {
				xAxisPoints.push(calPoints[0] && calPoints[0][i] && calPoints[0][i].x ? calPoints[0][i].x : currentList[i ? i - 1 : i] && currentList[i ? i - 1 : i].x);
			}
		}
		if ((opts.type == 'line' || opts.type == 'area') && opts.xAxis.boundaryGap == `${app.prefix}ui-justify`) {
...
function calCategoriesData(categories, opts, config, eachSpacing) {
  ...
  const categoriesTextLenth = categories.map(item => measureText(item, opts.xAxis.fontSize || config.fontSize));
...
function calXAxisData(series, opts, config) {
    ...
    const textLength = xAxisScaleValues.map(item => measureText(item));
...
function getFunnelDataPoints(series, radius) {
  const process = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  series = series.sort((a, b) =>  parseInt(b.data) - parseInt(a.data));
  for (let i = 0; i < series.length; i++) {
...
function getXAxisPoints(categories, opts, config) {
  ...
  if ((opts.type == 'line' || opts.type == 'area') && dataCount > 1 && opts.xAxis.boundaryGap == `${app.prefix}ui-justify`) {
    dataCount -= 1;
  }
  ...
  if (opts.xAxis.boundaryGap !== `${app.prefix}ui-justify`) {
...
function getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
  ...
  data.forEach((item, index) => {
    if (item === null) {
      ...
      if (typeof item === 'object' && item !== null) {
				if (item.constructor == Array) {
					let xranges; let xminRange; let  xmaxRange;
  ...
  if (YLength > 0) {
    ...
    for (let i = 0;i < YLength;i++) {
      const yData = opts.yAxis.data[i];
			...
			rangesFormatArr[i] = rangesArr[i].map((items) => {
    ...
    var yAxisWidthArr = new Array(1);
		...
		rangesFormatArr[0] = rangesArr[0].map((item) => {
...
function drawPointText(points, series, config, context) {
  ...
  points.forEach((item, index) => {
    if (item !== null) {
      ...
      if (series.suffix) value = `${value}${series.suffix}`;
...
function drawToolTipHorizentalLine(opts, config, context, eachSpacing, xAxisPoints) {
  ...
  if (toolTipOption.yAxisLabel) {
    ...
    for (let i = 0;i < labelText.length;i++) {
      ...
      let bgStartX; let bgEndX; let  bgWidth;
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
  ...
  const textWidth = textList.map(item => measureText(item.text, config.fontSize));
...
function drawColumnDataPoints(series, opts, config, context) {
  ...
  series.forEach((eachSeries, seriesIndex) => {
    let ranges; let minRange; let  maxRange;
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config._xAxisTextAngle_ === 0) {
      newCategories.forEach((item, index) => {
        let offset = - measureText(String(item), xAxisFontSize) / 2;
        if (boundaryGap == 'center') {
      ...
      newCategories.forEach((item, index) => {
        ...
        context.setFontSize(xAxisFontSize);
        context.setFillStyle(opts.xAxis.fontColor || '#666666');
        const textWidth = measureText(String(item), xAxisFontSize);
  ...
  series.forEach((eachSeries, seriesIndex) => {
    if (opts.tooltip) {
      if (opts.tooltip.index == seriesIndex) {
        ...
        context.arc(
centerPosition.x, centerPosition.y, eachSeries._radius_ + activeRadius, eachSeries._start_,
  ...
  series.forEach((eachSeries, seriesIndex) => {
    if (opts.tooltip) {
      if (opts.tooltip.index == seriesIndex) {
        ...
        context.moveTo(centerPosition.x, centerPosition.y);
        context.arc(
centerPosition.x, centerPosition.y, activeRadius + eachSeries._radius_, eachSeries._start_,
...
centerPosition.x - (getMaxTextListLength(opts.categories) + config.radarLabelTextMargin),
  ...
  const _loop = function _loop(i) {
    ...
    coordinateAngle.forEach((angle, index) => {
      const pos = convertCoordinateOrigin(radius / radarOption.gridCount * i * Math.cos(angle), radius / radarOption.gridCount * i * Math.sin(angle), centerPosition);
      if (index === 0) {
        startPos = pos;
...
function normalInt(min, max, iter) {
    ...
    return  Math.floor(arr.reduce((i, j) =>  i + j) / iter * (max - min)) + min;
};
...
function getWordCloudPoint(opts, type) {
  const points = opts.series.sort((a, b) =>  parseInt(b.textSize) - parseInt(a.textSize));
  switch (type) {
    case 'normal':
      for (let i = 0; i < points.length; i++) {
...
return false;
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        const tWidth = measureText(text, tHeight);
        const isSpin = Spin();
        let x; let y; let area; let  areav;
        let breaknum = 0;
        while (true) {
          breaknum++;
          let isCollision;
...
function drawFunnelText(series, opts, context, eachSpacing, labelAlign, activeWidth, centerPosition) {
  for (let i = 0;i < series.length;i++) {
    const item = series[i];
    let startX; let endX; let startY; let  fontSize;
    const text = item.format ? item.format(+item._proportion_.toFixed(2)) : `${util.toFixed(item._proportion_ * 100) }%`;
    if (labelAlign == 'right') {
      if (i == 0) {
...
this.opts.series[0].data, seriesData, this.opts.chartData.calPoints,
